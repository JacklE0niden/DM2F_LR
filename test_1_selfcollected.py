...
def parse_args():
    ...
    parser.add_argument(
        '--snapshot', type=str, default='version10_OHaze', help='snapshot to load for testing')
    ...
    with torch.no_grad():
        ...
        for name, root in to_test.items():
            ...
            for idx, data in enumerate(dataloader):
                ...
                haze, file_name = data
                print("Data loaded:", haze.shape, file_name)
                ...
                save_dir = os.path.join(ckpt_path, exp_name, f'({exp_name}) {name}_{args.snapshot}')
                os.makedirs(save_dir, exist_ok=True)  # 确保目录存在

                for r, f in zip(res.cpu(), file_name):
                    to_pil(r).save(os.path.join(save_dir, f'{f}_dehazed.png'))
